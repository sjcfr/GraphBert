...
def parse_opt_to_name(opt):
    ...
    loss_aa_distrib = opt.loss_aa_distrib[0]
    ...
    name = [str(n) for n in [pretrain, graph_type, model_type, model_size, start_layer, merge_layer, n_layer_extractor, n_layer_aa, n_layer_gnn, n_layer_merger, smooth_term, smooth_method, 
                             Lambda, layer_norm, method_extractor, method_merger, loss_aa_distrib, lr, warmup_proportion, test_type]]
    name = "_".join(name)
    return name
    ...
    for example_index, example in enumerate(examples):
        ...
        for ith_sent, sent in enumerate(context_sentences):
            if ith_sent == 0:
                ...
                sp_token = '[SEP]'
            sent_tokens = [sp_token] + tokenizer.tokenize(sent) + ['.']
        ...
        for ending_index, ending in enumerate(example['candidates']):
            ...
            ending_tokens = ["[SEP]"] + ending_tokens + ['.']
            
            sentence_ind_ending = [ith_sent + 1] * len(ending_tokens)
            
            _truncate_seq_pair(context_tokens_tmp, ending_tokens, max_seq_length)
            _truncate_seq_pair(sentence_ind_context_tmp, sentence_ind_ending, max_seq_length)
            
            tokens = context_tokens_tmp + ending_tokens
            
            segment_ids = [0] * len(context_tokens_tmp) + [1] * len(ending_tokens)
...
def ini_from_pretrained(config, graph_embedder=None):
    ...
    if config.trained_model is None:
        ...
        if config.graph_type:
            model_config.graph_type = config.graph_type
...
def loss_graph(appro_matrix, true_graph, loss_fn, smooth_term=0, distribution='cat'):
    if distribution == 'normal':
        
        _, graph_vector, graph_vector_post = appro_matrix
        ...
        appro_matrix = appro_matrix[0].squeeze()
...
def load_data(features, batch_size=32, dat_type='train'):
    ...
    input_tokens = select_field(features, 'tokens')
