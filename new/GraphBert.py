...
class GraphExtractor(nn.Module):
    def __init__(self, config):
        ...
        self.config.method = 'cross'
        self.config.layer_norm = config.layer_norm
        self.method = config.method_extractor
            ...
            for n in range(batch):
                ...
                for ith_sent in range(num_sent):
                    if self.method == 'mean':
                        ...
                        graph_vectors_sample_ith = start_layer[n][sent_ind[n] == ith_sent].max(0).unsqueeze(0)
...
class GraphBertEncoder(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, sentence_ind, true_adjacancy_matrix=None, output_all_encoded_layers=True, head_mask=None):
        ...
        which means that:
                                                       ...
                                                        |
                                              ____Merger_Layer____ 
                                                   |             |
                                              BERT_Layer         |
                                                   |             |
                                          ____Merger_Layer____   |
                                           |               |     |
        vacant layer(s)(at least one)  O O O O O           |     |
        sl + num_sub_layers            O O O O O----->GraphExtractor   sl + num_sub_layers + 1
        ...                            ...
        sl + 1                         O O O O O----->GraphExtractor   sl + 2  
        sl                             O O O O O----->GraphExtractor   sl + 1
        e.g.:15th is the start layer, num_sub_layers=2, merge_layer=18.
             means the 15, 16, 17th bert layer is the input of GraphExtractor, and the output of 18th bert layer(merge_layer) 
             together with the output of GraphExtractor is feed into merger.
        '''
        ...
        if true_adjacancy_matrix is not None:
            return all_encoder_layers, (attn_scores, graph_vectors, graph_vectors_post) # !!!
        else:
            return all_encoder_layers, (attn_scores, graph_vectors, graph_vectors)
...
class GraphBertModel(BertPreTrainedModel):
    ...
    def forward(self, input_ids, sentence_inds=None, graphs=None, token_type_ids=None, attention_mask=None, output_all_encoded_layers=False, head_mask=None):
        ...
        encoded_layers, (attn_scores, graph_vectors_post, graph_vectors) = encoded_layers
        ...
        if self.is_pretrain:
            ...
            return encoded_layers, cls_scores, (attn_scores, graph_vectors_post, graph_vectors)
